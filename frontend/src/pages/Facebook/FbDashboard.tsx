...
export function FbDashboard() {
  ...
  const saveSelectionMutation = useMutation({
    mutationFn: ({ viaId, ids }: { viaId: number; ids: number[] }) => savePageSelection(viaId, ids),
    onSuccess: (updated) => {
  ...
  const onSubmitCampaign: import("react-hook-form").SubmitHandler<CampaignForm> = (data) => {
    ...
    addCampaignMutation.mutate({
      ...
      via_account_id: (campaignViaId as any)?.toString() ?? null,
  ...
  return (
    <>
            ...
            {selectedViaId && (
              <div className="section-head-actions">
                {selectionDirty && (
                  ...
                  >
                    {saveSelectionMutation.isPending ? "Đang lưu..." : `Lưu (${selectedCount})`}
                  </button>
                )}
                <button
      ...
      {/* ── FB Automation Campaign ── */}
      <section className="glass-card panel">
        <div className="section-head">
          <div>
            <p className="eyebrow">Lên lịch tự động · Facebook</p>
        ...
        {fbCampaigns.length > 0 ? (
                  ...
                  <>
                    <div className="row-card-header">
                      ...
                      <div className="row-card-actions">
                        ...
                        <button className="btn btn-ghost btn-sm" onClick={() => setEditingCampaign(c)}>
                          Sửa
                        </button>
          ...
          !showAddCampaign && (
            <p className="empty-hint">
              Chưa có campaign Facebook nào. Tick chọn pages ở trên rồi nhấn "+ Tạo Campaign".
            </p>
          )
        )}
      </section>
...
function FbCampaignEditForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      via_account_id: (viaId as any)?.toString() ?? null,
      ...
      {viaId && (
        <div className="campaign-pages-section">
          ...
          {selectedPages.length === 0 ? (
            <p className="subtle" style={{ fontSize: 13, marginTop: 8 }}>
              Via này chưa có page nào được chọn.
            </p>
          ) : (
            <div className="campaign-pages-list">
              {selectedPages.map((p) => (
